use super::outcome::ModelToViewMessage;
use super::CreateStatus;
use application_models::statuses::NewStatus;
use application_models::statuses::StatusesRepo;
use async_trait::async_trait;
use design_scaffold::endpoint::Model;
use design_scaffold::oid::OidPool;
use design_scaffold::repo::Repository;
use sqlx::sqlite::Sqlite;

#[async_trait]
impl<'endpoint> Model<'endpoint, Sqlite, NewStatus, ModelToViewMessage> for CreateStatus {
    async fn model(
        &'endpoint self,
        option_oid_pool: Option<&OidPool>,
        db_conn_pool: &sqlx::Pool<Sqlite>,
        new_status: NewStatus,
    ) -> design_scaffold::Result<ModelToViewMessage> {
        let outcome_data = StatusesRepo::create(option_oid_pool, db_conn_pool, &new_status).await?;

        Ok(ModelToViewMessage::new(outcome_data))
    }
}
